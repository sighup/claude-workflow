...
The naming computation lives in `cw_worktree_names()` in `scripts/lib/cw-common.sh` (from a source checkout: `plugin/scripts/lib/cw-common.sh`); the `/cw-worktree create` protocol in `references/worktree-commands.md` step 1 mirrors that logic exactly.
