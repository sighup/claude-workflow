...
**External engine (any non-Claude model)**: Task()'s `model` parameter accepts Claude models only (`haiku`, `sonnet`, `opus`). When `metadata.model` is anything else (e.g. `"gpt-5.6-sol"`, `"gpt-5.6-terra"`), spawn `subagent_type: "claude-workflow:codex-implementer"` with `model: "sonnet"` and a description prefixed with the model value, keeping the same team template below (the wrapper participates in team messaging like any teammate). State the target model value and the task `complexity` verbatim in the prompt (the wrapper maps complexity→codex reasoning effort); its EXECUTION LOOP line 1 becomes "Run your codex protocol (falling back to cw-execute if codex is unavailable)". See [cw-codex](../cw-codex/SKILL.md); the wrapper passes the value to `codex exec -m` and records `model_used`/`fallback_reason` in its journal.
