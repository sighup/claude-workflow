...
```
SendMessage({ to: "worker-1", message: { type: "shutdown_request", reason: "All tasks complete. Shutting down." } })
SendMessage({ to: "worker-2", message: { type: "shutdown_request", reason: "All tasks complete. Shutting down." } })
... (for each teammate)
```
