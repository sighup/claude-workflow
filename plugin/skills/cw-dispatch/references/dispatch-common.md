...
**When the threshold is met:** remove `scripts/task-store-guard.sh`, remove the SessionStart hook entry in `.claude-plugin/plugin.json` that spawns the guard, and note the removal in the commit message. (Paths are plugin-root-relative; in a source checkout of this repo they live at `plugin/scripts/task-store-guard.sh` and `plugin/.claude-plugin/plugin.json` — commit the change there, not in the installed plugin cache.)
