...
- **NEVER** give workers direct implementation instructions — they **MUST** invoke `cw-execute`. The one exception is `codex-implementer` on its codex path: it drives the external engine per its agent protocol (mechanics: the [cw-codex](../cw-codex/SKILL.md) skill), and still invokes `cw-execute` itself whenever it falls back
...
Resolve the list id first — `CLAUDE_CODE_TASK_LIST_ID` when set; otherwise discover the session's actual list directory per [dispatch-common.md](references/dispatch-common.md#writer-lease-lifecycle). Never invent an id from the project or package name: a mis-keyed lease silently voids the guard's defer coordination for the real list.
...
**External engine (any non-Claude model)**: Task()'s `model` parameter accepts Claude models only (`haiku`, `sonnet`, `opus`). When `metadata.model` is anything else — `"gpt-5.6-sol"`, `"gpt-5.6-terra"`, or a future external model named in [model-selection.md](../cw-plan/references/model-selection.md) — never pass it to Task(). Spawn the wrapper instead: `subagent_type: "claude-workflow:codex-implementer"`, `model: "sonnet"`, and prefix the description with the model value (e.g. `description: "gpt-5.6-sol: Execute task T01"`) — the platform UI shows the wrapper's Claude model, so the label is the only visible indication of the real worker. The spawn prompt keeps the exact assignment shape below, replacing the first line with "Run your codex protocol to implement it", stating the target model value verbatim, and adding one line: `Codex mechanics: see the cw-codex skill (plugin skills/cw-codex/SKILL.md).` The wrapper passes the value straight through to the CLI (`codex exec -m "<model>"`), so adding a new external model is a rubric-table edit — no dispatcher changes. The wrapper degrades on its own — if the codex CLI is absent or fails, it executes the task itself via cw-execute on sonnet and records the fallback in its journal; the dispatcher never checks for codex.

**Workers hold no Task tools** — they cannot read the board. `TaskGet` the task once here and inline its **complete** assignment into the spawn prompt: `task_id`, `requirements`, `complexity`, `scope` (`files_to_create`, `files_to_modify`, `patterns_to_follow`), `proof_artifacts`, `proof_capture`, `spec_path`, `commit.template`, and `verification.pre`/`verification.post`. (`complexity` lets the codex wrapper scale its reasoning effort — trivial→low, standard→medium, complex→high.) A worker with stripped tools has no board fallback, so an incomplete prompt cannot be recovered — verify the serialized assignment is complete before spawning.
...
Workers hold no Task tools — they never mark themselves done. **After each batch joins, you harvest their durable evidence and apply every completion yourself.** For each joined worker's `task_id`, resolve its outcome by evidence order (RESULT BLOCK → `{task_id}.result.json` → proof-dir scan by `task_id`), verify the journal's `commit_sha` is reachable in git, then apply **one** `TaskUpdate(completed)` at a time. Apply the journal's `model_used` (and `fallback_reason`, when present) to the task's result metadata verbatim — a `gpt-5.6-sol` task that fell back records `model_used: "sonnet"` plus the reason, never the planned value — writing a harvest-checkpoint line **before** each and doing a `TaskGet` read-back **after** each. Never a burst. Full protocol: [dispatch-common.md](references/dispatch-common.md#harvest-and-apply).

### Step 5: Refresh, Monitor, and Report

1. **Refresh the lease** so the heartbeat advances each loop (a stale lease becomes reclaimable by another writer). Pass the token-file path again — this Bash invocation does not inherit anything from acquire time:
   ```bash
   CW_LEASE_TOKEN_FILE="docs/specs/<run>/results/dispatch-owner.token" \
