...
- **plugin**: Update path references broken by the `plugin/` directory move
  (`git rev-parse --show-toplevel` fallback in `cw-worktree`, semantic-release
  version path in `pyproject.toml`)
