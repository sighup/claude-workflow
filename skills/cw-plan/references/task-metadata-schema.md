...
Cost class is an **opt-in optimization for Phase 4**. Phase 5 always runs every entry as the gate, regardless of cost class — there is no Phase 5 skip path.

| Cost | Meaning | Worker behavior |
|------|---------|-----------------|
| `fast` | Completes in seconds (lint, typecheck, format, single-file unit tests) | MAY run incrementally during Phase 4 implementation for early feedback. Phase 5 still runs it as the gate |
