...
**Decision protocol:** Zero configuration. If your session is already running Fable, frontier mode is on — your model choice *is* the opt-in. Otherwise the first frontier-capable skill you run (research, spec, or plan) asks once per session ("Use Claude Fable 5 for frontier stages?"). Nothing is written to settings files: the answer is carried for the session, and once tasks are planned, each task's `metadata.model` on the board is the durable record that dispatch and workers read. Dispatch and execution never prompt.
...
**Cost:** Fable pricing is approximately 2× Opus; the tokenizer is ~30% heavier, so effective cost per task is roughly 2.6× Opus. Fable usage is included in subscription plans through 2026-06-22; after that it is credit-metered. The `complex`-tier tasks and subagent spawns (planner, spec writer, research deep-dive) request Fable when frontier mode is on; trivial and standard tasks, review, validation, and testing remain on the default ladder.

**Advisor alternative:** If you prefer not to route full tasks to Fable, you can use Claude Code's native advisor tool as a complementary alternative: set `"advisorModel": "fable"` in your `.claude/settings.json` (or pass `--advisor fable` / `/advisor fable`) to receive Fable guidance at decision points while a cheaper main model executes. This complements frontier routing rather than replacing per-task subagent `model` selection — users who decline full frontier routing get Fable insight without Fable execution costs. Requires Claude Code v2.1.170+ and Fable access. Note: `fable` is absent from the `/advisor` dropdown menu — you must pass it explicitly. A Fable main session accepts only a Fable advisor.

Nothing in the plugin requires Fable. The feature is fully backward-compatible: decline the one-time question (or run when Fable is unavailable) and behavior is identical to the previous release.
